...
function LightningStrike( keys )
	...
	local dmg 		= ability:GetLevelSpecialValueFor("damage", level)
	local dmg_int	= ability:GetLevelSpecialValueFor("int_damage", level)
	local final_dmg = caster:GetIntellect() * dmg_int + dmg

	ApplyDamage({victim = target, attacker = caster, damage = final_dmg, damage_type = ability:GetAbilityDamageType()})
    ...
    print(level)
    -- Second lightning
    if level >= 3 and RandomInt(0, 100) <= 33 then
    	Timers:CreateTimer(0.314, function()
    		ApplyDamage({victim = target, attacker = caster, damage = final_dmg, damage_type = ability:GetAbilityDamageType()})
			local lightningBolt = ParticleManager:CreateParticle(keys.particle, PATTACH_WORLDORIGIN, caster)
		    ParticleManager:SetParticleControl(lightningBolt,0,Vector(caster:GetAbsOrigin().x,caster:GetAbsOrigin().y,caster:GetAbsOrigin().z + caster:GetBoundingMaxs().z ))
		    ParticleManager:SetParticleControl(lightningBolt,1,Vector(target:GetAbsOrigin().x,target:GetAbsOrigin().y,target:GetAbsOrigin().z + target:GetBoundingMaxs().z ))
...
function LavaBurst(keys)
--	PrintTable("LavaBurst",keys)
	local caster	= keys.caster
	local target 	= keys.target
	local ability 	= keys.ability
	local level 	= ability:GetLevel() - 1
	...
	if target:HasModifier("modifier_shaman_fire_shock") then
...
function LightningJump(keys)
	...
	local ability 	= keys.ability
	local level 	= ability:GetLevel() - 1
	local radius 	= ability:GetLevelSpecialValueFor("bounce_radius", level)
	local dmg 		= ability:GetLevelSpecialValueFor("damage", level)
	local dmg_int	= ability:GetLevelSpecialValueFor("int_damage", level)
...
function ChainLightning(keys)
	...
	ability.jump_count[ability.instance] = ability:GetLevelSpecialValueFor("bounces", (ability:GetLevel() -1))
